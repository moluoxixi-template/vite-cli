...
- Set component-scoped tokens on the component that consumes them when the component defines its own default. For example, use `style="--el-header-padding: 0"` on `ElHeader`; setting the token only on an ancestor is shadowed by the header's default token.
...
<div :style="{ '--el-color-primary': themeColor }">
  <ElHeader style="--el-header-padding: 0">
...
- generated headers use the public padding token and compute to zero padding at runtime;
