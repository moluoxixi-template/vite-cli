...
测试多个模块组合的功能，验证 feature 组合的正确性。

| 测试项 | 验证内容 |
|-------|---------|
| 文件生成 | 应该生成的文件都存在，不应该生成的文件不存在 |
| 依赖完整性 | 必需的依赖都存在，不应该存在的依赖不存在 |
| Catalog 解析 | 所有 `catalog:` 引用都被替换为实际版本号 |
| Import 路径 | 没有跨模板引用（如 `../../templates/`） |
| package.json | 结构完整，元数据正确 |
...
测试多个模块组合在一起的功能，验证 feature 组合的正确性。

**测试内容：**
- 文件生成完整性（应该生成的文件 vs 不应该生成的文件）
- 依赖完整性（必需的依赖 vs 不应该存在的依赖）
- Catalog 引用解析（确保所有 `catalog:` 引用都被替换为实际版本号）
- Import 路径验证（确保没有跨模板引用）
- package.json 结构验证
